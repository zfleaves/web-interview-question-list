...
// 读取主文件
const mainFile = path.join(__dirname, 'CSS面试题集锦.md');
...
// 创建问题文件夹
const questionsDir = path.join(__dirname, 'questions-css');
...
// 创建索引文件
const indexContent = `# CSS 面试题集锦（截止 2025 年底）

## 目录

${questions.map((q, i) => `${i + 1}. [${i + 1}. ${q.title}](./questions-css/${String(i + 1).padStart(2, '0')}-${i + 1}__${q.title.replace(/[\/\\:*?"<>|？]/g, '_').replace(/\s+/g, '_')}.md)`).join('\n')}
...
[查看详细答案](./questions-css/${String(i + 1).padStart(2, '0')}-${i + 1}__${q.title.replace(/[\/\\:*?"<>|？]/g, '_').replace(/\s+/g, '_')}.md)`).join('\n')}
`;
