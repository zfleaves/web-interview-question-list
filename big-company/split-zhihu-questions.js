...
const mainFile = path.join(__dirname, '知乎面试题集锦.md');
const outputDir = path.join(__dirname, 'questions-zhihu');
const indexFile = path.join(__dirname, 'index-zhihu.md');
...
${questions.map(q => {
  const safeTitle = q.title.replace(/[\/\\:*?"<>|?。,、（）\(\)]/g, '_').replace(/\s+/g, '_');
  return `${q.number}. [${q.number}. ${q.title}](./questions-zhihu/${String(q.number).padStart(2, '0')}__${q.number}__${safeTitle}.md)`;
}).join('\n')}
