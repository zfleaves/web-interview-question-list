...
const mainFile = path.join(__dirname, '大厂面试题集锦.md');
const outputDir = path.join(__dirname, 'questions-big-company');
const indexFile = path.join(__dirname, 'index-big-company.md');
...
${questions.map(q => {
  const safeTitle = q.title.replace(/[\/\\:*?"<>|？。,、（）\(\)]/g, '_').replace(/\s+/g, '_');
  return `${q.number}. [${q.number}. ${q.title}](./questions-big-company/${String(q.number).padStart(2, '0')}__${q.number}__${safeTitle}.md)`;
}).join('\n')}
