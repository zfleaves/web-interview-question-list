...
const mainFile = path.join(__dirname, '腾讯面试题集锦.md');
const outputDir = path.join(__dirname, 'questions-tencent');
const indexFile = path.join(__dirname, 'index-tencent.md');
...
${questions.map(q => {
  const safeTitle = q.title.replace(/[\/\\:*?"<>|？。,、（）\(\)]/g, '_').replace(/\s+/g, '_');
  return `${q.number}. [${q.number}. ${q.title}](./questions-tencent/${String(q.number).padStart(2, '0')}__${q.number}__${safeTitle}.md)`;
}).join('\n')}
