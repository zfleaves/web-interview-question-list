...
const mainFile = path.join(__dirname, '字节面试题集锦.md');
const outputDir = path.join(__dirname, 'questions-bytedance');
const indexFile = path.join(__dirname, 'index-bytedance.md');
...
${questions.map(q => {
  const safeTitle = q.title.replace(/[\/\\:*?"<>|？。,、（）\(\)]/g, '_').replace(/\s+/g, '_');
  return `${q.number}. [${q.number}. ${q.title}](./questions-bytedance/${String(q.number).padStart(2, '0')}__${q.number}__${safeTitle}.md)`;
}).join('\n')}
