...
const mainFile = path.join(__dirname, '百度面试题集锦.md');
const outputDir = path.join(__dirname, 'questions-baidu');
const indexFile = path.join(__dirname, 'index-baidu.md');
...
${questions.map(q => {
  const safeTitle = q.title.replace(/[\/\\:*?"<>|？。,、（）\(\)]/g, '_').replace(/\s+/g, '_');
  return `${q.number}. [${q.number}. ${q.title}](./questions-baidu/${String(q.number).padStart(2, '0')}__${q.number}__${safeTitle}.md)`;
}).join('\n')}
