...
// 读取主文件
const mainFile = path.join(__dirname, 'Vite高频面试题补充.md');
...
// 创建问题文件夹
const questionsDir = path.join(__dirname, 'questions-vite-supplement');
...
// 生成索引文件
const indexContent = `# Vite 高频面试题补充（截止 2025 年底）

## 目录

${questions.map(q => {
  const safeTitle = q.title.replace(/[\/\\:*?"<>|？。,、（）\(\)]/g, '_').replace(/\s+/g, '_');
  return `${q.number}. [${q.number}. ${q.title}](./questions-vite-supplement/${String(q.number).padStart(2, '0')}__${q.number}__${safeTitle}.md)`;
}).join('\n')}
...
[查看详细答案](./questions-vite-supplement/${String(q.number).padStart(2, '0')}__${q.number}__${safeTitle}.md)`;
}).join('\n')}
`;

fs.writeFileSync(path.join(__dirname, 'index-vite-supplement.md'), indexContent, 'utf-8');
console.log('创建文件: index-vite-supplement.md');
